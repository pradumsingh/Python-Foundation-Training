...
class Law_Enforcement_Agencies(DBConnection):
    ...
    def update_table(self):
        try:
            ...
            update_query = 'update law_enforcement_agencies set agency_name=%s, jurisdiction=%s, phone_num=%s where agency_id=%s'
            data = (self.agency_name, self.jurisdiction, self.phone_num, self.agency_id)
            DBConnection.getConnection()
            stmt = DBConnection.connection.cursor()
